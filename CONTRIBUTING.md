...
This project needs Elixir with `mix` (2025 solutions) and Python with `uv` (2024 solutions), plus `just` as the task runner.
...
| --- | --- |
| `default` | Lists all available recipes |
| `install` | Installs dependencies for both stacks |
| `install-ex` | Installs Elixir dependencies |
| `install-py` | Installs Python dependencies |
| `format` | Formats code in both stacks |
| `format-ex` | Formats Elixir code |
| `format-py` | Upgrades Python syntax to 3.13 and formats it |
| `lint` | Lints the Python code (Elixir has no lint recipe) |
| `lint-py` | Lints the Python code |
| `test` | Runs tests for both stacks |
| `test-ex` | Runs Elixir tests |
| `test-py` | Runs Python tests |
| `check` | Runs `lint` then `test` |
| `update` | Updates Python dependencies (Elixir has no update recipe) |
| `update-py` | Upgrades the `uv` lockfile and syncs it with `uv-upsync` |
| `benchmark-ex-solution YEAR DAY PART` | Benchmarks a single Elixir solution with `hyperfine` |
| `benchmark-ex-year YEAR` | Benchmarks every solved Elixir day/part pair for a year |

1. Fork the repository and clone your fork locally
2. Create a branch following the naming pattern described below
